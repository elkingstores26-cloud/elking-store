...
window.renderProductCard = function(p) {
  ...
  // inStock defaults to true for legacy products without the field set.
  const outOfStock = p.inStock === false;
  return `
  <a href="product.html?id=${p.id}" class="product-card bg-white rounded-3xl border border-black/5 overflow-hidden flex flex-col group cursor-pointer relative ${outOfStock ? 'opacity-70' : ''}">
    <div class="relative aspect-[4/3] bg-cream overflow-hidden">
      <img class="product-img w-full h-full object-cover ${outOfStock ? 'grayscale' : ''}" src="${p.img}" alt="${p.name}" loading="lazy" />
      <span class="absolute top-4 right-4 ${tagColor} text-[11px] font-extrabold px-3 py-1 rounded-full tracking-wide">${tagText}</span>
      ${outOfStock
        ? `<span class="absolute top-4 left-4 bg-black/85 text-white text-[12px] font-extrabold px-3 py-1 rounded-full shadow-lg">غير متوفر</span>`
        : (hasDiscount
          ? `<span class="absolute top-4 left-4 bg-red-600 text-white text-[12px] font-extrabold px-3 py-1 rounded-full shadow-lg">خصم ${pct}٪</span>`
          : (badge ? `<span class="absolute top-4 left-4 bg-white/95 text-ink text-[11px] font-bold px-3 py-1 rounded-full">${badge}</span>` : ''))}
      ${outOfStock ? `<div class="absolute inset-0 flex items-center justify-center bg-black/30">
        <div class="bg-white/95 text-ink text-[13px] font-extrabold px-5 py-2 rounded-full shadow-lg">⛔ نفد المخزون</div>
      </div>` : ''}
    ...
    <div class="p-6 flex flex-col flex-1">
      ...
      <div class="flex items-end justify-between mt-auto pt-5 border-t border-black/5">
        ...
        ${outOfStock
          ? `<span class="inline-flex items-center gap-1.5 bg-black/10 text-black/50 font-bold px-4 py-2.5 rounded-full text-[13px] cursor-not-allowed">غير متوفر</span>`
